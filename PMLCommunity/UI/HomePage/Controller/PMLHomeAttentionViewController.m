...
@interface PMLHomeAttentionViewController ()<PMLAutoScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,JPTableViewPlayVideoDelegate,MLVideoWriteDelegate, PMLCommentInputKViewDelegate>{
    int _currentPage;
}
...
@implementation PMLHomeAttentionViewController
- (void)dealloc
{
...
static void Callback(CFRunLoopObserverRef observer, CFRunLoopActivity activity, void *info){
    ...
    PMLHomeAttentionViewController * vc = (__bridge PMLHomeAttentionViewController *)info;
    if (vc.tasks.count == 0) {
        return;
    }
    BOOL result = NO;
...
//                [cell.contentImageView setImage:videoModel.preViewImage];
                [cell.contentImageView setImage:vModel.base64Image];
                cell.isVideo = true;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self addTask:^BOOL{
        [PMLHomeAttentionViewController addContentWithtableView:tableView Cell:cell withIndexPath:indexPath model:model];
        return true;
    }];
    return cell;
}
